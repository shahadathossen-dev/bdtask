...
@section('content')
    <div
        ...
        <h2 class="text-lg font-medium text-left text-gray-500 border-b border-gray-200 rounded-t-lg bg-gray-50 dark:border-gray-700 dark:text-gray-400 dark:bg-gray-800 p-4"
            id="defaultTab" data-tabs-toggle="#defaultTabContent" role="tablist">Adjustments</h2>
                ...
                @if ($adjustments->count())
                    @foreach ($adjustments as $adjustment)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td scope="col" class="px-6 py-3">{{ $adjustment->joining_date }}</td>
                            <td scope="col" class="px-6 py-3">{{ $adjustment->name }}</td>
                            ...
                            <td scope="col" class="px-6 py-3 flex">
                                <a href="{{ url("adjustments/$adjustment->id/edit") }}"
                                ...
                                &nbsp;
                                <form action="{{ url("adjustments/$adjustment->id") }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
